...
(function(window, document, Math) {
    ...
    DatePicker.prototype = {
        ...
        FillData: function() {
            ...
            var title = self.options.title ? self.options.title : '';
        ...
        eventClick: function() {
            ...
            document.querySelector('.picker-ok').addEventListener("touchstart", function(e) {
                ...
                e.preventDefault();  
                // 隐藏按月筛选
                document.querySelector(".select-head-cont").style.display = "none"
                document.querySelector(".month").style.display = "block"
                document.getElementById("nowValue").style.display = "block"
                var inputValue = '';
                var inputValueTwo = '';
                for (var i = 0; i < self.textArray.length; i++) {
                    if (i == 0) {
                        inputValue += self.textArray[i].value;
                        console.log("年", inputValue)
                        var syear = inputValue;

                    } else {
                        inputValue += self.options.separator + self.textArray[i].value;
                        // inputValueTwo = inputValue + "月";
                        console.log("inputValue", inputValue)
                        window.sessionStorage.setItem("inputValue", inputValue+"月")
                        window.sessionStorage.setItem("inputValue2", inputValue)
                        var smouth = self.textArray[i].value
                        console.log("月", self.textArray[i].value)
                    }
                }
                // 取到日期并调用vue中的方法
                var sdate = syear + '-' + smouth;
                console.log("sdate", sdate)
                window.sessionStorage.setItem("sdate", sdate)
                app.pageContent(1, sdate);
                app.changeTime(inputValue);
                //回调函数
